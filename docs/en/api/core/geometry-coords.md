...
- [Geometry: interpolate](/en/api/core/geometry-interpolate) — primary `LngLat[]` consumer/producer
- [Geometry: anchorConvert](/en/api/core/geometry-anchor-convert) — uses `toLngLat` / `toGeoPoint` for `BezierAnchor` ↔ `BezierAnchorData`
- [Apollo compile: projection](/en/api/core/geometry-apollo-compile) — `projectPoint` / `unprojectPoint` for metre-space work
