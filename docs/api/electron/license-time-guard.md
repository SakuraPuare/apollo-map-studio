...
- [License crypto](/api/electron/license-crypto) — `aesEncrypt`, `getFileKey`, `getMacKey`
- [License manager](/api/electron/license-manager) — calls `trustedNow()` and `snapshot()`
- [License storage](/api/electron/license-storage) — same encryption pattern
