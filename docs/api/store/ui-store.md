...
See [/api/store/store-ui](/api/store/store-ui) for the legacy
compatibility entry.
...
- [/api/store/map-store](/api/store/map-store) — undoable counterpart.
- [/api/store/settings-store](/api/store/settings-store) — persisted
  scalar preferences.
- [/api/core/geometry/snap](/api/core/geometry/snap) — `SnapTarget`
  type.
