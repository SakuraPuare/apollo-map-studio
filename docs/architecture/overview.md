...
| `src/io/proto/loader`             | Bundled `.proto` loading from `src/proto/` via protobufjs                               |
...
Apollo `.proto` files live under `src/proto/` and are bundled into the Vite app
with `import.meta.glob(..., { query: '?raw' })`. `protobufjs` parses those raw
strings through a custom `fetch` implementation, so map import/export works
offline in web and desktop builds.
